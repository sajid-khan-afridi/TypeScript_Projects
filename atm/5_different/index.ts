...
(async () => {
  let userLogin: { userName: string; userPass: string } = await inquirer.prompt(
    [
      {
        name: "userName",
        type: "input",
  ...
  let atmOption: {
    option: "Balance Inquiry" | "Cash Withdrawl" | "Transfer Money" | "Exit";
  ...
  switch (atmOption.option) {
    ...
    case "Cash Withdrawl":
      console.log(`Your Current balance is ${userData.userAmount}`);
      let withdraw: { amount: number } = await inquirer.prompt({
        name: "amount",
        type: "number",
